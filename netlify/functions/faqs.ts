...
export default async (req: Request, context: Context) => {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (pageId) items = items.filter((f: any) => f.page_id === pageId);
    ...
    if (req.method === 'POST') {
      ...
      const newItem = { id: generateId(), ...body, order: body.order ?? items.length, created_at: new Date().toISOString() };
      items.push(newItem);
    ...
    if (req.method === 'DELETE' && pathParts.length === 1) {
      const id = pathParts[0];
      const items = await getData('faqs');
      await setData('faqs', items.filter((i: any) => i.id !== id));
