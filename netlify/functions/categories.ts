...
function createHandler(entityKey: string) {
  return async (req: Request, context: Context) => {
    ...
    const pathParts = url.pathname.replace(`/api/${entityKey}`, '').replace(/^\/|\/$/g, '').split('/').filter(Boolean);

    try {
      if (req.method === 'GET') {
        const items = await getData(entityKey);
      ...
      if (req.method === 'DELETE' && pathParts.length === 1) {
        const id = pathParts[0];
        const items = await getData(entityKey);
        await setData(entityKey, items.filter((i: any) => i.id !== id));
