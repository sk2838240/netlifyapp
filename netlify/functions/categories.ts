...
export default async (req: Request, context: Context) => {
  ...
  const pathParts = url.pathname.replace('/api/categories', '').replace(/^\/|\/$/g, '').split('/').filter(Boolean);

  try {
    // GET /api/categories
    if (req.method === 'GET') {
      const items = await getData('categories');
    ...
    if (req.method === 'POST') {
      ...
      items.push(newItem);
      await setData('categories', items);
    ...
    if (req.method === 'PUT' && pathParts.length === 1) {
      ...
      if (index === -1) {
        return new Response(JSON.stringify({ message: 'Category not found' }), { status: 404, headers });
      }
    ...
    if (req.method === 'DELETE' && pathParts.length === 1) {
      ...
      await setData('categories', filtered);

      return new Response(JSON.stringify({ success: true, message: 'Category deleted' }), { headers });
    }
